...
var cssRuleEndLine = /\}\s*$/;
var cssEnd = /css$/;

var getScope = function getScope(cursor) {
  var desc = cursor.getScopeDescriptor();

  var scopeString = desc.scopes.pop();
  var scope = scopeString.split('.').pop();
...
module.exports = {
  ...
  endLine: function(terminator, insertNewLine) {
    ...
    return editor.getCursors().forEach(function(cursor) {
      ...
      var scope = getScope(cursor);

      editor.moveToEndOfLine();

      if (!terminator && cssEnd.test(scope)) {
        terminator = objectLiteralLine.test(line) && !cssRuleEndLine.test(line) ? ';' : '';
      } else if (!terminator) {
        terminator = objectLiteralLine.test(line) ? ',' : ';';
      }

      if (!continuationLine.test(line) && !emptyLine.test(line) && scope !== 'html') {
